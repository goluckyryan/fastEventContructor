...
/**///==============================================================   
   ...
   if (tree->GetBranch("tracePara")) {
      tree->SetBranchAddress("tracePara", tracePara);
      isTraceParaExist = true;
   }
   if (tree->GetBranch("traceChi2")) {
   ...
   para_default[0] = 1000; // Amplitude
   para_default[1] = 50; // Time
   para_default[2] = 10; // Rise time
   para_default[3] = 4000; // baseline
   ...
   for( int ev = 0; ev < totnumEntry; ev++){
      ...
      for( int j = 0; j <  traceCount ; j++){
         ...
         g->SetTitle(Form("ev: %d, nHit : %d, id : %d, trace Length : %u\n", evID, j, idDet,traceLength[j]));
         g->GetXaxis()->SetTitle("tick = 10 ns");
      
         fit->SetRange(0, traceLength[j]);
         // fit->SetParameters(para_default);
         fit->SetParameter(0,  -1000); // set amplitude
         fit->SetParameter(1, 50); // set time
         fit->SetParameter(2, 30); // set rise time
         fit->SetParameter(3, trace[j][100]); // set baseline

         // fit->SetParLimits(1, 10, 100);
         // fit->SetParLimits(2, 20, 40);
         ...
         TString fitResultStr2;
         if ( isTraceParaExist ){
            fitResultStr2 = Form("Amp: %.2f, Time: %.2f tick, Rise time(10%%-90%%): %.3f ticks, baseline : %.2f", 
